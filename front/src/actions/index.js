...
export const updateButton = number => {
  ...
  return { type: "UPDATE_BUTTON", number };
};
  ...
  return dispatch => {
    ...
    return (
      ...
      axios
        ...
        .then(
          axios.spread((eventRes, locationsRes) => {
            ...
            eventsLoire.map(event => {
              console.log(event, event.location_id, event.post_id);
              const location = locs.find(
            ...
            dispatch(updateEventsList(eventsLoire));
          })
        )
        .catch(e => {
          console.log(e);
